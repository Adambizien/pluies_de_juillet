          ...
          {registrations
            ...
            return (
              <div key={registration.id} className={`bg-white rounded-lg shadow-md overflow-hidden ${eventPast ? 'opacity-60' : ''}`}>
                <div className={`${eventPast ? 'bg-gray-500' : 'bg-indigo-600'} text-white px-6 py-4`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-2xl font-bold">
                        {event.title}
                        {eventPast && <span className="ml-3 text-sm font-normal">(Événement terminé)</span>}
                      </h2>
                      <p className={`${eventPast ? 'text-gray-200' : 'text-indigo-100'} text-sm mt-1`}>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-2">
                                <span className={`px-2 py-1 ${eventPast ? 'bg-gray-200 text-gray-600' : 'bg-purple-100 text-purple-700'} text-xs font-semibold rounded-full`}>
                                  {conference.category.name}
                                </span>
                                <span className={`text-sm ${eventPast ? 'text-gray-400' : 'text-gray-500'}`}>
                                  {formatDateTime(conference.startDatetime)} - {formatDateTime(conference.endDatetime)}
                                </span>
                              </div>
                              <h3 className={`text-lg font-semibold ${eventPast ? 'text-gray-500' : 'text-gray-900'} mb-1`}>
                                {conference.title}
                              </h3>
                              <p className={`${eventPast ? 'text-gray-400' : 'text-gray-600'} text-sm`}>
